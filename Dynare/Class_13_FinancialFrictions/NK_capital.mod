...
var			y c inv k h r_k w z R Pi mc Delta p_tilde Num Den lambda d q;
varexo		eps_z eps_d eps_R;
parameters	alffa betta delta eta kappa mu phi theta sigmma
			rho_z rho_d
	...
	kappa	= 0.5;		// investment adjustment cost
...
model;
	...
	// (3) Asset markets
	q		= betta * lambda(+1)/lambda * (r_k(+1)+(1-delta)*q(+1));
	// (4) Real wages
	...
	// (12) Investment
	k		= (1-kappa/2*(inv/inv(-1)-1)^2)*inv + (1-delta)*k(-1);
	...
	// (17) Price of capital q
	1		= q*(1-kappa/2*(inv/inv(-1)-1)^2-kappa*(inv/inv(-1)-1)*inv/inv(-1)) + betta*lambda(+1)/lambda*q(+1)*kappa*(inv(+1)/inv-1)*(inv(+1)/inv)^2;
end;
